        ...
        <div className="flex flex-col w-full max-w-6xl mx-auto px-4 mt-8 min-h-screen">
            <div className="flex flex-col mb-6 ml-4">
                ...
                <p className="text-[#575881] dark:text-gray-300 font-semibold">
                    {carregando ? "Carregando seus imóveis..." : `Você possui ${imoveis.length} imóvel(is) cadastrado(s).`}
                </p>
            </div>

            <Main>
                {!carregando && imoveis.length === 0 ? (
                    <div className="text-center py-20 text-gray-500">Você ainda não tem nenhuma locação cadastrada.</div>
                ) : (
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {imoveis.map((imovel) => (
                            <div key={imovel.id} className="bg-white dark:bg-[#1f1f25] rounded-3xl shadow-md border border-gray-100 dark:border-gray-800 p-5 flex flex-col gap-4">
                                <div className="flex justify-between items-start">
                                    <h2 className="font-bold text-lg text-[#2A2B51] dark:text-white line-clamp-2">
                                        {imovel.dadosGerais?.titulo}
                                    </h2>
                                    <span className={`text-xs font-bold px-3 py-1 rounded-full ${imovel.aprovado ? 'bg-green-100 text-green-700' : 'bg-yellow-100 text-yellow-700'}`}>
