    ...
    return (
        <>
                ...
                <div className="flex flex-row w-full max-w-6xl mx-auto px-4 py-12 gap-10 justify-center">
                    <div>
                        <Image className="rounded-xl object-cover w-[500px] h-[300px] mt-9 shadow-lg transition-all duration-300" src={imagemPrincipal} alt={imovel.imgAlt} width={500} height={300} />
                    </div>
                    <div className="flex flex-col gap-4">
                        ...
                        <div ref={carrosselRef} className="flex flex-col gap-4 overflow-y-auto h-[300px] [&::-webkit-scrollbar]:hidden snap-y snap-mandatory scroll-smooth">
                            {imagens.map((imgSrc, index) => (
                                <div 
                                    key={index}
                                    onClick={() => setImagemPrincipal(imgSrc)}
                                    className={`shrink-0 snap-center cursor-pointer border-4 rounded-xl transition-all duration-300 ${imagemPrincipal === imgSrc ? "border-blue-500 scale-105" : "border-transparent hover:scale-105"} `}
                                >
